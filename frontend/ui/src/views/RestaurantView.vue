  ...
  <div class="flex flex-col h-full bg-brand-light font-sans antialiased">
    ...
    <header class="fixed top-0 w-full bg-brand-dark text-white shadow-lg p-4 flex justify-between items-center z-10">
      ...
      <div class="absolute left-1/2 top-full -translate-x-1/2 -translate-y-1/2">
        <img src="/src/assets/sumi.png" alt="Sumi-IA Avatar" class="w-24 h-24 sm:w-32 sm:h-32 rounded-full">
      </div>

      <!-- Info de Usuario a la Derecha -->
      <div v-if="authStore.isAuthenticated" class="flex items-center gap-1 sm:gap-4">
        <span class="text-brand-cream hidden md:inline text-sm">{{ authStore.userName }}</span>
    ...
    <footer class="fixed bottom-0 w-full bg-brand-light border-t border-brand-cream p-4" v-if="authStore.isAuthenticated">
...
const saveConversationToFirestore = async (question: string, answer: string) => {
  ...
  try {
    await saveConversationToRestaurant(restaurantSlug, {
...
const handleConsentUpdate = (consentRecord: any) => {
